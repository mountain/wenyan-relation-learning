...
import { core, sha256, findLine } from './catalog.mjs';
const SCHEMA = 'wenyan.changes.learning-event.v1';
const FEATURES = ['bits', 'position', 'polarity', 'lower', 'upper', 'central', 'proper', 'oppositeAtCorrespondence'];
  ...
  for (const event of events) {
    ...
    if (p?.kind === 'profile') {
      ...
      if (!exact(p, ['kind', 'profile', 'context', 'label', 'source']) || !profiles.has(p.profile)
          || !profiles.get(p.profile).labels.includes(p.label)
          || !exact(p.context, ['bits', 'position'])) throw new Error('invalid labelled example');
...
export function predictContext(events, profileId, context, catalog, fuel = 100000) {
  ...
  const query = core.lineContext(context.bits, context.position);
  ...
  const usable = examples.filter(e => !excluded.has(`${e.context.bits}:${e.context.position}`));
  ...
  for (const features of profile.hypotheses) {
    ...
    for (const e of usable) {
      ...
      checked++;
      const c = core.lineContext(e.context.bits, e.context.position);
