...
## 禁止混淆（forbidden_conflations）

字段与做法借自 `adva/docs/claims.toml`：Adva 给**每一条** claim 附一份「不得作出的混淆」清单。
本仓库原先只有散文式的 `nonclaims`——它说的是「什么没被建立」，而不是「哪两个不同的东西
不得被当成一个」。

**本仓库迄今查出的每一个严重缺陷都是一次混淆**，而且每次都先产出过错误成品才被发现。
把它们命名，就能断言它们：

```
node tools/knowledge/conflations.mjs [--write] [--json]
```

退出码：`0` 全部成立；`1` 观察到混淆；`2` Unknown（读不到必需产物，故无判据，**不得当作通过**）。

| id | 禁止的混淆 | 见证（本仓库真实发生） |
|---|---|---|
| C1 | **认得构式** ≠ **语法接受这条标注** | 两次：先经 `constructionOf()` 对多匹配段落返回构式，后因二槽帧让此类段落由罕见变常见。门报 PASS 而模型根本建不起来 |
| C2 | **门通过** ≠ **每个已声明构式都被检查过** | `REPORTING_PROBES` 只声明原 4 个，新增 3 个（含独占语料 20.57% 的 `yue-quote`）从未被查，G2 却绿了一整轮 |
| C3 | **声明了构式** ≠ **写好了它预设的那份契约** | 契约里写着二槽帧须「等一份单独的契约先说清缺失角色的含义」，我先用了它——前提一直躺在文件里，动手前没读 |
| C4 | **报告的分母** ≠ **构式自己的排列空间** | readiness 一律打印 `survivors=n/6`，而二槽构式只排列两个角色，`/6` 把已定的映射显示成未定 |
| C5 | **来源可回查** ≠ **标注为真** | `verifySources()` 长期报 29/29，而这个数**对标注内容什么也没说**；11 条存活记录的槽根本无法如此陈述（`於子貢`、`於是武王遍`、`周公乃`、`敖者`） |
| C6 | **契约里写了规则** ≠ **代码在执行它** | 本文件。检查器审计自己的声明，使「声明」永远不能超出「强制」 |

### 设计要点

- **单一真源**：可采纳性判据在 `tools/grammar/admissibility.mjs`，seeder（拒绝写入）与
  存活库审计（C5）共用。规则只被一方知道时就不是规则。
- **检查器必须能失败**：C1 断言同一条坏记录**在无谓词时会被收进核心**——否则该检查什么也没证明。
  反向验证做过：给 contract 塞一个不存在的 `C99`，退出码 1 并指名该文件。
- **检不了就如实说检不了**：C2 对 v1 语法报 **UNCHECKED**，因为 `relations.ts` 不导出构式清单，
  而它是 patch 文件（加导出会改动上游交付物）。退出码 2 专为这种情形而设。

### 不声称

- 不声称这六条穷尽了本仓库的混淆；它们是**已经发生过**的那些。
- 不声称 C5 证明标注在语言学上正确：它只证明没有存活记录带**已声明规则拒绝**的跨度。
