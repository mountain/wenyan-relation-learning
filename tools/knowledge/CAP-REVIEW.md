...
# 补篇：把语料变成证据（2026-09 后续实测）

## 一、G2/G3 的缺口只有一处，且与语料无关

G2 的全部失败原因是 **v1 三条构式各只有 1 条标注**（floor 2）；reporting 语法
（唯一吃语料的那条）本已通过。按 §实测一 的结论补足最小可证伪集的第 2 条后，
G2/G3 对两条语法同时通过。

实体取 **壬/癸/琴**：刻意避开 fixture 训练集（甲乙书）与其声明的三组留出泛化
测试集（丙丁帛、戊己玉、庚辛简），否则会把 `heldOutEntities: 9` 变成假命题。
**这不证明关于文本的任何知识**：v1 句式在语料中命中 0 次，它证明的是机器。

## 二、G8 的 0.5 实测不可达

G8 = 用语法读**每一段**（分母 77,884），Q1 返回 Answered 的比例，地板 0.5。

| 构式族 | 实测覆盖 |
|---|---|
| 原 4 帧 `实体+动词+实体+曰：「…」` | 2,154 段（2.77%） |
| 松上界（段落中出现任一关系动词） | 52,608（67.55%） |
| 较严上界（指人主语+报道动词） | 35,729（45.87%） |

«松上界」只统计**动词出现**，不是**能解析出框架**，因此不能充数。
真实可达的是下表。

## 三、两槽构式是唯一的大头，而它要求角色模型承认「未表达」

`子曰：「學而時習之。」` 有名有引语、**无受话人**。写出一个受话人是编造，
返回 Unknown 又丢掉了文本确实给出的施事与主题。故角色可为 `null`
（未表达），并由构式**声明自己表达哪些角色**。

实测各帧覆盖（重叠即弃，按「标记最全者优先」排序）：
...
1. `孔子謂弟子曰：「…」` **同时**命中三槽帧与二槽 `E曰` 帧（后者读成 agent=弟子）。
   若算作两处匹配，则每个三槽段落都成「歧义」，**29 条既有语料记录全部作废**。
2. `王問曰：「何谓也？」` 被「无曰的三槽帧」静默吃掉，**recipient 槽里装的是 `曰`**。

修法：帧按**标记数量**从多到少排序，**与已占跨度重叠的匹配一律丢弃**。

## 五、多匹配：代价测得后，规则被改

严格规则（多匹配即 Unknown）下：旧 4 帧可读 2,154 段；7 帧可读 8,726 段（11.20%）；
另有 **8,323 段（10.69%）**因多匹配被放弃。

该代价一旦测出，原规则就不再成立：一段同时写着「子曰…」与「王曰…」的文本**陈述了
两个关系**，拒绝读其一并不更诚实。改为**读标记最全的一帧，并报告其余帧的数量**
（`additionalMatches`）——**不需要契约变更**（Q1 仍返回一条关系，只是读数不再假装
该段只有一帧）。解除后可读 **16,679 段（21.42%）**。

更要紧的是：`selectOperationalCore` 原先只检查 `constructionOf(text)` 是否为真，
而多匹配段落**仍然会返回构式名**，于是它们进了核心；`replay` 是全有全无，
**一条**这样的记录就让整条语法抛 `out-of-grammar evidence`。
这正是 §实测三 描述的失效，**从另一扇门重新进来**：二槽帧让多框架段落从罕见变成常见。
现在「在语法内」定义为**唯一解析**，多匹配记录进 `outOfGrammar` 由 G9 记账。

## 六、标注的可采纳性：不是所有槽都能当标签

原 `seed-reporting.mjs` 是**先到先得**，于是语料立刻供给了一批不能诚实标注的槽：

```
於是武王遍告諸侯曰  → agent「於是武王遍」      厲聲謂曰 → agent「厲聲」
樅公相謂曰         → agent「樅公相」          野語有之曰 → agent「野」
齊士有皇子告敖者曰   → agent「齊士有皇子」      且告曰   → agent「且」
```

把这些标为施事是**断言假命题**。因此 seeder 增加**声明的可采纳性过滤**，
并报告拒绝率：7 个构式在单匹配下可读的 8,726 段中，**1,002 段（11.48%）**
的 agent 槽以助词/副词开头。

**据此撤回了 6 条既有记录**（agent 槽为 `又`/`若有`/`以`/`傳以`/`且`），
撤回理由具名，记录本身不删除——这正是撤回机制存在的场合。
另有两个二槽构式（`謂曰`、`語曰`）因真实文本形式多被副词/助词占据
（`厲聲謂曰`、`蓋其語曰`、`故諸儒爲之語曰`）而**不予声明**：宁可少一个构式，
不可多一条假标注。
...
|---|---|---|
| reporting 构式数 | 4 | 7 |
| 语料覆盖（G5） | 2,133（2.74%） | **16,679（21.42%）** |
| G8 可答率 | 4.47% | **21.42%** |
| G2 / G3 | FAIL | **PASS** |
| G8 | FAIL | **FAIL（如用户所定，地板不调低）** |

## 八、不声称

- 不声称 21.42% 是上限：⑷⑸ 两个「无曰」帧族实测还值约 1.5 个百分点，未声明是因为
  其分段质量未测。
- 不声称 0.5 可达：**较严上界**（指人主语+报道动词）实测 45.87%，**低于地板**；
  松上界 67.55% 只统计「动词出现」，不是「能解析出框架」。故 G8 在**当前角色契约**下
  不可达，这是测得的结论，而地板按纪律不调低。
- 不声称已解决分段质量：11.48% 的可读段落 agent 槽受助词污染，这是**测得的**缺陷率。
- 不声称 G8 应当被满足；本次只是把「达不到」从断言变成了**带数字的结论**。
- 不声称两槽构式的 `recipient: null` 是语义学结论；它是**声明**：该构式不表达受话人。

---

# 补篇二：G8 的 0.5 实测不可达（最后一条路径已测完）

目标②要求把语料可答率提到 0.5。本轮把**最后一条可能补上缺口的路径**也测了，
结论是该门槛在**当前角色契约与「不调低门槛」的约束下不可达**。

## 已有覆盖的构成（分母 77,884 段）

| 来源 | 段落 | 占比 |
|---|---|---|
| reporting 语法（7 构式） | 16,679 | **21.42%** |
| 已声明的上界（出现任一关系动词） | 52,608 | 67.55% |
| 已声明的较严上界（指人主语+报道动词） | 35,729 | 45.87% |

关键在读法：**上界统计的是「动词出现」，不是「能解析出框架」**。
45.87% 这个数**低于地板**，而它是「较严」的上界——真能解析的只会更少。

## 最后一条路径：转移类构式（授/與/賜/獻/遺…）

报道框架的主题由 `「」` 免费定界；转移框架**没有定界符**，所以只有两种可能：
找**显式角色标记**（切分有依据），或靠猜（切分无依据）。

**有显式标记的**（`以` 标主题、`於` 标受事、`之` 代受事）：
...
**无标记的裸双宾** `X V Y T`：18,548 段（23.81%）——但主题跨度**没有任何依据**，
只能假设「先受事后主题」，实测 17.96% 的施事槽已是虚词/副词。
这正是 seeder 的可采纳性过滤要拒绝的东西，**拿到了也不能当证据**。

## 结论：缺口 ~25.6 个百分点，且没有诚实手段可补

```
地板                50.00%
诚实可达（当前）      21.42%
+ 有标记的转移框架     ≈ 24.4%   ← 最后一条有依据的路径，仅 +2.97pp
────────────────────────────
缺口                ≈ 25.6pp
```

要跨过它，只有三条路，每条都与目标自身的纪律冲突：

1. **无标记双宾**：+23.81pp，但切分是猜的 → 违反③「不伪造标注」。
2. **大规模论元结构标注**：需要人力监督的语料，本仓库没有，也不是本轮能造出来的。
3. **换问题或降门槛**：②明写「**不调低门槛**」。

因此 G8 的 FAIL **不是「还没做够」，而是「按现在的定义做不到」**。
本仓库的做法是把这件事**带数字地记下来**，而不是把门槛挪到当前值上。

## 不声称

- 不声称 24.4% 是理论极限：它是**已声明框架清单**的极限，不是文言文关系结构的极限。
- 不声称 45.87% 与 50% 的关系意味着「只差一点」：前者不是可解析框架的计数。
- 不声称 G8 应当被废除；它记录的是**这个系统现在做不到会话性推理**，这个结论本身有用。

---

# 补篇三：把「混淆」变成可断言的对象

G8 的门槛已判定不可达（补篇二）。此后本仓库的一类工作才真正开始有回报：
**不再增加覆盖率，而是把已经犯过的错误变成机器能拒绝的东西。**

借自 `adva/docs/claims.toml` 的 `forbidden_conflations`：Adva 给每条 claim 附一份
「不得作出的混淆」清单。本仓库补上这一字段后，六条混淆各有一个检查
（`tools/knowledge/conflations.mjs`），并当场查出两个新问题：

1. **C5 抓到一条存活记录**（`corpus:zhuangzi:19.23:gao-quote`）带 `敖者` 这个受事槽。
   它**逃过了我自己前一轮的撤回扫描**——那次扫描用 Python 的 `re.match()`，
   从**字符串开头**锚定，所以交替式里的 `者$` 分支永远不会触发。
   这是审计的缺陷，不是规则的缺陷；规则此前只被 seeder 知道，
   一旦成为单一真源并施于存活库，问题立刻现形。**「规则只被一方知道」本身就是漏洞。**
2. **C2 报出未检查项**：v1 语法的探针覆盖**检不了**，因为 `relations.ts` 不导出构式清单，
   而它是 patch 文件。检查器把这一项报为 UNCHECKED 并给理由，退出码 2 专为此设——
   「检不了」不得与「通过了」共用一种输出。

## 一条设计原则，值得写下来

**检查器必须被证明会失败。** 因此 C1 不只断言坏记录被拒，还断言它
**在没有一致性谓词时会被收进核心**——若不成立，该检查什么也没证明。
反向验证也做过：给 contract 塞一个不存在的 `C99`，检查器退出 1 并指名该文件与 id。

一个永远通过的门，和一个不存在的门，读数上无法区分。
