...
## 五、多匹配的代价，以及它如何再次攻破「全有全无」

严格规则（多匹配即 Unknown）下：旧 4 帧可读 2,154 段；7 帧可读 **8,726 段（11.20%）**；
另有 **8,323 段（10.69%）**因多匹配被放弃——这是**下一个可动的杠杆**，但它要求
Q1 返回「一组关系」而非一条，属契约变更，本步未做。
...
|---|---|---|
| reporting 构式数 | 4 | 7 |
| 语料覆盖（G5） | 2,133（2.74%） | **8,720（11.20%）** |
| G8 可答率 | 4.47% | **11.20%** |
| G2 / G3 | FAIL | **PASS** |
...
- 不声称 11.20% 是上限：解除多匹配规则可再得约 10.69 个百分点，但那需要契约变更。
