...
const GRAMMAR = [
  ...
  { id: "hand-over", pattern: new RegExp(`^${SLOT}把${SLOT}交给${SLOT}。?$`, "u") },
  { id: "receive", pattern: new RegExp(`^${SLOT}获${SLOT}所授${SLOT}。?$`, "u") }
];
...
function validRoles(r: Roles) {
  return r && Object.keys(r).sort().join(",") === "agent,recipient,theme"
    && ROLE_KEYS.every(k => typeof r[k] === "string" && r[k].length > 0);
}
function replay(model: RelationModel) {
  if (!model || model.schema !== "wenyan.relations.v1"
...
export function readRelation(text: string, model: RelationModel): RelationReading {
  const rules = replay(model);
  const p = parse(text);
  if (!p) return { status: "Unknown", reason: "outside-controlled-grammar",
 ...
 */
export function learnRelations(model: RelationModel, example: Example, fuel: number) {
  const rules = replay(model);
  ...
  for (const order of candidates) {
    if (checked === fuel) return { status: "Unknown", reason: "fuel-exhausted",
      model, checked, refutations };
    checked++;
  ...
  if (!survivors.length) return { status: "Conflict", reason: "contradictory-label",
    model, checked, refutations };
  // The label is a declared supervisor constraint, not an inferred historical fact.
  const next: RelationModel = JSON.parse(JSON.stringify({ ...model,
...
export function compareRelations(a: string, b: string, model: RelationModel) {
  const left = readRelation(a, model), right = readRelation(b, model);
  if (left.status === "Unknown" || right.status === "Unknown")
    return { status: "Unknown", left, right };
  const x = left.candidates[0], y = right.candidates[0];
